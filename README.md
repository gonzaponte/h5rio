...
- Iterator helper for reading table datasets row by row.
...
`iter_table` reads a one-dimensional table dataset row by row. This is useful
when a table is too large to load fully with `read_table`.

```rust
use h5rio::{h5type, iter_table};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    for row in iter_table::<Hit>("hits.h5", "/hits")? {
        let hit = row?;
        println!("sensor {} charge {}", hit.sensor_id, hit.charge);
    }
...
pub fn iter_table<T>(filename: &str, dataset: &str) -> hdf5::Result<Hdf5TableIter<T>>
...
`read_table` and `read_array` load complete datasets into memory. `iter_table`
keeps the file open and yields one row at a time. `iter_array` keeps the file
open and yields one array entry at a time.
...
- complete table and array reading;
- row-by-row table reading;
- entry-by-entry array reading;
